...
Update to use completion metadata:
...
export function renderPointsBadge(
  ...
  // For COMPLETED tasks: Show actual points earned
  ...
  // For INCOMPLETE tasks: Predict bonus
  if (task.parent_uid && task.status === "needs_action") {
    ...
    if (template && template.streak_bonus_points && template.streak_bonus_interval) {
      // Use instance's streak snapshot to predict bonus
      const streakWhenCreated = task.streak_when_created || 0;
      const nextStreak = streakWhenCreated + 1;  // What streak will be after completing this
      
      if (nextStreak % template.streak_bonus_interval === 0) {
        // Completing this will award bonus!
        return html`<span
...
- [ ] Late completions no longer receive bonuses
...
## 1. Feedback on: "For INCOMPLETE tasks: Predict bonus"
> I think this section fails to address the "Late Completions Still Get Bonuses" bug. It's only looking at the streak when created, but that could have been days ago. Is that correct or am I missing something?
