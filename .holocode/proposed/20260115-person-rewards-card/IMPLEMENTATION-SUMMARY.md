...
- [x] Bundle size < 200 KB (actual: 166.60 KB)
- [x] All 6 cards registered in bundle
...
- [ ] Falls back to `default_person_entity` if configured
- [ ] Falls back to first person alphabetically
- [ ] Dropdown animates smoothly (grid transition)
- [ ] Selected person highlighted with checkmark
- [ ] Avatar shows image or initials
- [ ] Points display uses custom terminology
- [ ] Chevron icon rotates when opening/closing

**Rewards List**:
- [ ] Only shows rewards for selected person
- [ ] Sorting works (cost/name/created)
- [ ] Disabled rewards hidden unless config enabled
- [ ] Clicking reward opens confirmation modal
- [ ] Confirmation modal shows correct data
- [ ] Edit button opens edit modal with existing data
- [ ] Redeem button disabled if insufficient points
- [ ] Redemption deducts points correctly
- [ ] Confetti animation plays on success

**Progress Bar**:
- [ ] Shows task completion for selected person
- [ ] Updates when person changes
- [ ] Only counts dated tasks (excludes dateless)
- [ ] Percentage calculation correct

**Styling**:
- [ ] Accent color inherited from person profile
- [ ] Manual `accent_color` config overrides profile
- [ ] `hide_person_background` removes person section bg
- [ ] `hide_rewards_background` removes rewards section bg
- [ ] 20px gap between sections
- [ ] Responsive on mobile (< 600px)
- [ ] Avatar shrinks to 48px on mobile
...
1. **Manual Testing Required**: Runtime behavior not yet validated in live HA environment
2. **No Unit Tests**: TypeScript compilation verified, but no Jest/Vitest tests written
3. **CSS Duplication**: ~750 lines of CSS copied (cannot extract due to LitElement scoping)
4. **Single Bundle Only**: Cards cannot be loaded independently (all-or-nothing)
...
### Immediate (User)
1. Deploy bundle to Home Assistant
2. Add card to Lovelace dashboard
...
- Add quick redeem mode (skip confirmation)
- Rewards summary in person dropdown (# affordable)
- Achievement badges for redemption milestones

---

## References

- **Spec**: `.holocode/person-rewards-card/SPEC.md`
- **Related Specs**: 
